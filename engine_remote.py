...
Kept as a fallback when the local Stockfish pool is unavailable (binary
missing, startup error, etc.). The primary engine path is
`engine_pool.py`.
...
log = logging.getLogger("coach.engine.remote")
